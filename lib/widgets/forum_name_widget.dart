...
class ForumNameWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: Padding(
        padding: const EdgeInsets.only(top: 24.0, left: 10.0, right: 20.0, bottom: 5.0),
        child: Text(
          forum.title,
...
class CustomShapeBorder extends ShapeBorder {
  ...
  Path getOuterPath(Rect rect, {TextDirection textDirection}) {
    return getClip(Size(145.0, 60.0));
  }
  ...
  Path getClip(Size size) {
    ...
    clippedPath.quadraticBezierTo(0 + controlPointDistanceFromWall, 0 + controlPointDistanceFromWall,
        0, 0 + distanceFromWall);
    ...
    clippedPath.quadraticBezierTo(size.width - 1,
        size.height * 0.6 - distanceFromWall, size.width - distanceFromWall,
